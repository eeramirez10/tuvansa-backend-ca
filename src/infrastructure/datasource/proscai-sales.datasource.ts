...
import { SalesDto, SalesFilterDtoInput } from "../../domain/dto/sales/sales.dto";
...
export class ProscaiSalesDatasource extends SalesDatasource {



  async getSales(filters: SalesDto): Promise<SalesEntity[]> {
    ...
    if (dto.date) {
      ...
      params.push(
        dto.startDate.toISOString().slice(0, 10),
        dto.endDate.toISOString().slice(0, 10)
      );
    }
    ...
    return rows.map((r) => SalesMapper.jsonToEntity(r))
  }
}