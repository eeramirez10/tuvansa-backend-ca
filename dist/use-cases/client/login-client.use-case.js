...
class LoginClientUseCase {
    ...
    execute(loginUserDto) {
        return __awaiter(this, void 0, void 0, function* () {
            const client = yield this.authRepository.login(loginUserDto);
            if (!client)
                throw custom_error_1.CustomError.BadRequest('Client not found');
            const { id, password } = client, rest = __rest(client, ["id", "password"]);
            const token = yield this.signToken({ id });
            if (!token)
                throw custom_error_1.CustomError.internalServer('Error generating token');
            return {
                token,
                client: Object.assign({ id }, rest)
            };
        });
    }
}
